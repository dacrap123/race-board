...
export const playStart   = () => {
  // The setup button and the state broadcast can both request the cue in
  // single-device mode; prevent them from stacking two fanfares.
  if (Date.now() - lastStartAt < 5000) return;
  lastStartAt = Date.now();
  playLocalStartClip();
};

// Play the supplied local recording from the Start Race click and stop it at
// six seconds. Using an HTMLAudioElement keeps this in the browser's gesture
